...
export async function generateMetadata(): Promise<Metadata> {
  const blocks = await prisma.contentBlock.findMany({
    where: { key: { in: ['seo_courses_title', 'seo_courses_description'] } }
  });
  const content = Object.fromEntries(blocks.map(b => [b.key, b.value]));
  return {
    title: content.seo_courses_title || "Courses | Fiz",
    description: content.seo_courses_description || "Learn beatmaking and freestyle techniques from Fiz.",
  };
}
